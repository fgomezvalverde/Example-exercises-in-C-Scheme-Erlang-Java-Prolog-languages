...
(define Minimizar
  ...
  (cond((null? lista)1)
  (else(+
        (/(suma lista0) (car lista))(Minimizar lista0 (cdr lista)))))))

(define Minimax
 (lambda (lista0 lista)
    (cond((null? lista)(cons(list 0)'()))
     (else(cons(list          (with-handlers (
                                                [exn:fail:contract:divide-by-zero? (lambda (v) 99999)]
                                                )(/(/(suma lista0) (car lista))(Minimizar lista0 lista0)))(Minimax lista0 (cdr lista)))'())))))
...
(define elegir
  (lambda (lista cont)
    (cond((zero? (car(cdr(car lista))))(- cont 1))
...
(define arreglo
  (lambda (poblacion lista)
    (cond [(null? lista) (list poblacion)]
          [else
           (cons (car lista) (arreglo poblacion (cdr lista)))
           ])))
(define seleccion
  (lambda (poblacion cantidad lista0 lista cont numero elitismo)
    (arreglo (car (seleccion_aux poblacion cantidad lista0 lista cont numero elitismo)) (second (seleccion_aux poblacion cantidad lista0 lista cont numero elitismo)))
    ))
    
(define seleccion_aux
   (lambda (poblacion cantidad lista0 lista cont numero elitismo)
     (cond((equal? cantidad 0)'())
          ((equal? elitismo 'si)(append(append(list(list-ref poblacion (- (posicion (Minimax lista0 lista)) 1)) (seleccion_aux poblacion (- cantidad 1) lista0 lista numero cont 0)))))
          (else (cons (list-ref poblacion (elegir (union lista0 lista numero) cont )) (seleccion_aux poblacion (- cantidad 1) lista0 lista numero cont 0))))))
  ...
  (lambda (individuo evaluador)
    (cond [
          ...
          [
           ...
           (+(with-handlers (
                              [exn:fail:contract:arity? (lambda (v) 999999999)])
                                                (with-handlers (
                                                [exn:fail:contract:divide-by-zero? (lambda (v) 999999999)]
                                                [exn:fail:contract?  (lambda (v) 999999999)]
...
(define imprime_mejor
  (lambda (poblacion evaluador)
    ...
    ;(grafica (eval (append '(lambda (x))  (list (list-ref poblacion (imprime_mejor_aux evaluador -1 0 0))))))
   (with-handlers (
                              [exn:fail:contract:arity? (lambda (v) -1)])
                                                (with-handlers (
                                               [exn:fail:contract:divide-by-zero? (lambda (v) -1)]
                                                [exn:fail:contract?  (lambda (v) -1)]
                                                ) (grafica (eval (append '(lambda (x))  (list (list-ref poblacion (imprime_mejor_aux evaluador -1 0 0))))))))
...
(principal 15 10 "prueba1.txt" 'si)
